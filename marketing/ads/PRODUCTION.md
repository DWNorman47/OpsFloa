...
The `field-to-payroll` composition currently includes a generated 110 BPM original music bed and scene-timed Juniper narration generated with ElevenLabs. The narration is split into nine clips so individual lines can be replaced without changing the edit. Regenerate the music bed with `npm run audio:field-payroll`. For temporary local drafts only, replace the narration with the Windows system voice by running `npm run audio:field-payroll:system-voice`.
