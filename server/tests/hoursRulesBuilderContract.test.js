...
const BUILDER_OUTPUT = [
  ...
  { id: 'rg5h6i', type: 'add_time', when: { kind: 'weekdays', days: [1, 2, 3, 4, 5] }, edge: 'after', base: 'schedule', mode: 'at', anchor: 'clock', minutes: 30, at: '17:25' },
  { id: 'rj7k8l', type: 'add_time', when: { kind: 'weekdays', days: [1, 2, 3, 4, 5] }, edge: 'after', base: 'schedule', mode: 'every', anchor: 'clock', minutes: 30, from: '17:25', everyMin: 25 },
  { id: 'rsc1sc', type: 'add_time', when: { kind: 'weekdays', days: [1, 2, 3, 4, 5] }, edge: 'after', base: 'schedule', mode: 'at', anchor: 'schedule', minutes: 30, offsetMin: 25 },
  { id: 'rsc2sc', type: 'add_time', when: { kind: 'every_day' }, edge: 'after', base: 'schedule', mode: 'every', anchor: 'schedule', minutes: 30, offsetMin: 25, everyMin: 30 },
  { id: 'rm9n0o', type: 'remove_time', when: { kind: 'every_day' }, edge: 'before', base: 'punch', mode: 'at', anchor: 'clock', minutes: 15, at: '06:00' },
...
describe('every rule the builder can emit survives the engine', () => {
  ...
  test('every rule type the builder offers is one the engine knows', () => {
    ...
    const uiTypes = ['clip_start', 'clip_end', 'add_time', 'remove_time', 'auto_break', 'round', 'ot_tier',
      'rest_day', 'min_daily', 'sick_value', 'seventh_day', 'night_diff', 'window_mult'];
