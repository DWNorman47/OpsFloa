...
const SETTINGS_DEFAULTS = {
  ...
  // Set when an admin explicitly confirms the default rate / time zone from the
  // dashboard onboarding checklist (OnboardingChecklist.jsx). ISO timestamp.
  onboarding_rates_confirmed_at: '', onboarding_timezone_confirmed_at: '',
  ...
  paycheck_rules: '',
  // Default markup % by estimate category, as a JSON map e.g. {"materials":20,"subs":10}.
  // Applied by the catalog estimate-line resolver when an item has no explicit sell
  // price and no item-level markup — so costs can be set once at the company level.
