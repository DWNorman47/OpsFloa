...
| planroom/app.js | Count takeoff: `COUNT_PRESETS`, `readCountCfg`, `countResultRows`, `askCountConfig` | Count-takeoff section | verbatim; qcount markup = points + cfg (scale-independent); count = number of points |
| planroom/app.js | Vector wand: `matMul`, `matApply`, `flattenCubic`, `buildPathIndex`, `stitchChain`, `wandTrace`/`wandPickPath` (nearest-path pick + prompt) | Vector-wand / `wandPick` section | verbatim extraction/stitch; reads `state.doc.raw.getPage` at viewport scale 1 (Plan Room base px = markup space, so NO existing/proposed align-invert on the click); cache keyed by docKey:page; creates a `contour` markup in the current surface. Boundary clip (`clipToBoundary`) dropped |
| planroom/app.js | `wandArea` (auto-area) | — | **NEW, no sitework counterpart** — sitework's `autoAreaClick` is only a hatch-detect stub. Reuses `wandPickPath`; if the stitched run is a closed loop, opens the area form → qarea |
