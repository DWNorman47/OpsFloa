...
| `qbo_bill_range_pay.kind` | `daily_floor`, `weekly_guarantee`, `sick`, `vacation` | **enforced** (CHECK `chk_qbo_bill_range_pay_kind` in `0211`) | `server/constants/qboEnums.js` (`QBO_BILL_RANGE_PAY_KINDS`), `server/routes/qbo.js` (`billLabor`) | Ledger of range-level pay billed to contractors on QuickBooks Bills, one row per worker + kind + date (`pay_date` = the week's start for `weekly_guarantee`). Each bill posts current − ledgered amount, so overlapping ranges never double-bill or skip leave / guarantee / min-daily floors. |
