...
| `users.rate_type` | `hourly`, `daily` | **enforced** (CHECK in `0101`) | `server/constants/userEnums.js`, `server/routes/admin.js:1331` | Daily-rate pay calc + `day_mark_mode` gate. Since `0209` this column is the **current-rate cache** of `worker_rate_history.rate_type` (the row in effect today, company time zone); pay reads the dated history. |
| `worker_rate_history.rate_type` | `hourly`, `daily` | **enforced** (CHECK `chk_worker_rate_history_rate_type` in `0209`) | `server/constants/userEnums.js` (`USER_RATE_TYPES`), `server/utils/rateHistoryStore.js` (`validateChange`) | Effective-dated worker rate type. The pay engine prices each entry by the type in effect on its `work_date` (`server/utils/rateHistory.js`), so an hourly→daily switch mid-period prices each day under its own type. Omitted on an add → the type already in effect that day. |
...
| `users.worker_type` | `employee`, `contractor`, `subcontractor`, `owner`, `unpaid` | **enforced** (CHECK in `0071`, `unpaid` added in `0161`) | `server/constants/userEnums.js` (`USER_WORKER_TYPES`) | Display + report filtering + QBO routing (contractor/subcontractor → 1099 Vendor). **`unpaid`** = tracked but earns nothing: excluded from every pay surface via the `buildPayStatement` guard + list filters (payroll run, pay stubs, worker invoices, OT report, payroll CSV, certified payroll, scheduled pay email, QBO time sync). The payroll CSV, overtime report and QBO payroll journal share ONE worker set (`server/utils/payStatement.js` `payrollWorkers`): approved time/leave in range (any active flag) or active with a weekly guarantee, never `owner` / `unpaid`, and role `admin`/`super_admin` only with their own rate > 0 in effect during the range (a `worker_rate_history` row effective on/before the range end and not superseded on/before its start; no history at all → current `hourly_rate > 0`) — there is no salaried flag. |
...
| `reimbursements.status` | `pending`, `approved`, `rejected` | **enforced** (CHECK in `0071`) | `server/routes/reimbursements.js` (`REIMB_TRANSITIONS`) | Financial workflow. Admin transitions (`manage_reimbursements`): pending → approved \| rejected, rejected → pending, approved → pending only while `qbo_purchase_id` and `qbo_bill_id` are NULL and the expense date is in no locked pay period / finalized payroll run for the worker; anything else is 409 `invalid_transition`. Same-status saves (notes) allowed. A worker may delete only `pending` rows never pushed to QuickBooks. |
...
| `qbo_bill_range_pay.kind` | `worked`, `daily_floor`, `weekly_guarantee`, `sick`, `vacation` | **enforced** (CHECK `chk_qbo_bill_range_pay_kind` in `0211`, `worked` added in `0214`) | `server/constants/qboEnums.js` (`QBO_BILL_RANGE_PAY_KINDS`), `server/routes/qbo.js` (`billLabor`) | Ledger of what's been billed to contractors on QuickBooks Bills, one row per worker + kind + date (`pay_date` = the week's start for `weekly_guarantee`). `worked` = that DAY's worked pay (straight + OT + prevailing + night), so a backdated raise bills the difference for already-billed days on the next bill (owner decision 2026-09-24), like range pay. Each bill posts current − ledgered amount, so overlapping ranges never double-bill or skip leave / guarantee / min-daily floors. |
| `qbo_bill_range_pay.status` | `billed`, `baseline` | **enforced** (CHECK `chk_qbo_bill_range_pay_status` in `0214`, default `billed`) | `server/constants/qboEnums.js` (`QBO_BILL_LEDGER_STATUSES`), `server/routes/qbo.js` (`billLabor`) | `billed` = on a QuickBooks bill. `baseline` = lazily seeded cutover row: pay on a day/week billed BEFORE the ledger existed (entries flagged `time_entries.qbo_pre_ledger_bill` — a boolean frozen ONCE by `0218` from `qbo_synced_at` < `0211`'s `applied_at`, because `qbo_synced_at` is rewritten by other flows; nothing writes the flag after) is recorded as already billed at the amount computed when first met, instead of being billed again. Both count as billed in the diff. Companion column `credit_cents` (≤ 0, CHECK in `0214`): a negative adjustment held back because the bill would net negative — nets against the worker's next positive bill WHATEVER its range (a "Credit applied" line; a billed `worked` day whose entries are no longer approved is carried the same way), or `POST /api/qbo/bill-credits/record` applies it after a manual vendor credit. |
| `qbo_bill_pushes.status` | `pending`, `posted`, `mismatch`, `discarded` | **enforced** (CHECK `chk_qbo_bill_pushes_status` in `0214`, `mismatch`/`discarded` added in `0218`) | `server/constants/qboEnums.js` (`QBO_BILL_PUSH_STATUSES`, `QBO_BILL_PUSH_OPEN_STATUSES`), `server/routes/qbo.js` (push-bills, `reconcilePendingBills`, `/bill-outbox`) | Bill outbox. Written `pending` with the Intuit requestid + payload BEFORE `createBill`; `posted` when entry stamps + ledger commit in one transaction. A `pending` row is replayed with the same requestid on the next push (Intuit returns the existing bill) — never billed twice. A failed REPLAY never deletes it (the first send may have created the bill); only a definite rejection of the FIRST send deletes the row. `mismatch` = QuickBooks returned a bill (id kept in `qbo_bill_id`) whose total differs. `pending` + `mismatch` are open: they block the worker until a replay succeeds or an admin resolves them via `POST /api/qbo/bill-outbox/:id/resolve` — `confirm` (with the QuickBooks bill id → `posted`) or `discard` (no bill in QuickBooks → `discarded`; the content bills afresh under a new request id). Request ids are never reused across bills (per-worker sequence + collision check on closed rows). Companion `force` (0218): forced re-push, may restamp already-billed entries. |
...
| `settings.value` (key=`materials_cost_basis`) | `issued`, `received` | **app-only** | `server/constants/projectMoneyEnums.js` (`MATERIALS_COST_BASES`), `server/routes/admin.js` PATCH validation, `server/utils/projectCost.js` (`materialsCents`) | How a company recognizes MATERIALS cost on a project. `issued` (default) = inventory issued to the job (warehouse flow); `received` = value received on the job's POs (buy-to-jobsite flow). Open POs are committed in both; only the SPENT source switches, so no double-count. |
...
| `daily_reports.status` | `draft`, `submitted`, `reviewed` | **enforced** (CHECK in `0100`, was wrong in `0071`) | `server/routes/dailyReports.js` (POST create/upsert, PATCH, PATCH `/review`) | Daily-report workflow + edit lock. `0071` had `approved` instead; `0100` corrects to `reviewed`. POST writes only `draft`/`submitted` from the body (default `draft`); `reviewed` is set only by the admin review endpoint. A `reviewed` report is locked for non-admins: POST upsert, PATCH and DELETE all refuse it. |
...
| `field_report_photos.media_type` | `photo`, `video` (nullable) | **enforced** (CHECK in `0193`, `NOT VALID`) | `server/constants/fieldReportEnums.js` (`FIELD_REPORT_MEDIA_TYPES`), `server/routes/fieldReports.js` (coerced to allow-list on write) | Whether a field-report attachment is an image or a video (the client renders anything ≠ `video` as an image). Was app-only with no CHECK until `0193`. |
...
| `time_off_requests.status` | `pending`, `approved`, `denied`, `revoked` | **enforced** (CHECK in `0071`, `revoked` added in `0219`) | `server/constants/timeOffEnums.js` (`TIME_OFF_STATUSES`), `server/routes/timeOff.js` | PTO approval workflow. pending → approved \| denied (admin, `approve_entries`); approved → `revoked` (admin `PATCH /time-off/:id/revoke`, reason required → `revoke_reason`/`revoked_by`/`revoked_at`, audit-logged, un-flags the shifts the approval marked). Only `approved` is paid. `pending` + `approved` requests of one worker may not overlap (409 on submit and approve); `denied`/`revoked` don't block. A worker may delete only `pending`. |
...
| `estimate_lines.line_type` | `base`, `allowance`, `alternate`, `optional` | **enforced** (CHECK in `0186`) | `server/constants/projectMoneyEnums.js` (`ESTIMATE_LINE_TYPES`), `server/routes/estimates.js` | Bid line role. `base` + `allowance` count toward the bid total and the converted budget; `alternate` + `optional` are priced but shown separately (excluded from both). Default `base`. |
| `project_expenses.status` | `planned`, `actual` | **enforced** (CHECK in `0180`) | `server/constants/projectMoneyEnums.js` (`PROJECT_EXPENSE_STATUSES`), `server/routes/projectSpend.js`, `server/utils/projectCost.js` | Expense lifecycle. `actual` (default) = incurred → SPENT bucket; `planned` = a forecast (e.g. an equipment rental you intend to make) → COMMITTED bucket, like an issued-but-unpaid PO. Both the spend snapshot and P&L/WIP (`projectCost.manualExpensesByStatus`) split on it. `PATCH …/expenses/:id {status:'actual'}` converts a forecast to spent. |
...
| `client_documents.doc_type` | `w9`, `w2`, `coi`, `contract`, `license`, `other` (nullable) | **enforced** (CHECK in `0191`, `NOT VALID`) | `server/constants/clientDocumentEnums.js` (`CLIENT_DOCUMENT_TYPES`), `server/routes/admin.js` client-docs upload/create | Client compliance-doc category. Was app-only via two duplicated inline literals until `0191` added the CHECK + shared constant. NOTE: superset of the sibling `subcontractor_documents.doc_type` (adds `w2`). |
| `client_documents.direction` | `from_client`, `from_company` | **enforced** (CHECK in `0046`) | `server/routes/admin.js` client-docs upload | Who the document is from (client-supplied vs company-issued). |
| `inventory_locations.type` (physical `inventory_locations` table) | `warehouse`, `job_site`, `truck`, `other` (nullable) | **enforced** (CHECK in `0191`, `NOT VALID`) | `server/constants/inventoryEnums.js` (`INVENTORY_LOCATION_TYPES`), `server/routes/inventory.js` location create/update | Physical stock-location kind. Distinct from the JSON `inventory_items.locations[].type` row below (same value set, different column). Was app-only via a third inline copy until `0191`. Column DEFAULT is `warehouse` (constant `_DEFAULT` is `other` — a benign mismatch). |
| `purchase_orders.status` | `draft`, `submitted`, `partial`, `received`, `cancelled` | **enforced** (CHECK in `0050`) | `server/routes/inventory.js` (`PO_STATUSES` / `PO_MANUAL_TRANSITIONS`; PO receive/cancel flows) | Inventory purchase-order lifecycle. Drives received-basis materials cost + committed-cost rollup (cancelled excluded). `partial`/`received` are set by the receive route (only when that call actually received something). Manual PATCH edges: draft→submitted/cancelled, submitted→draft/cancelled, partial→received (close short)/cancelled, cancelled→draft; `received` is terminal. Never back to `draft` once any line has `qty_received > 0`, and a PO with receipts is never hard-deleted. Transition map is app-only (no trigger); no shared constant file yet. |
| `inventory_transactions.type` | `receive`, `issue`, `transfer`, `adjust`, `convert` | **enforced** (CHECK in `0038`, widened in `0043`) | `server/routes/inventory.js` | Stock-movement ledger kind. `issue` feeds project material cost. No shared constant yet. |
| `inventory_cycle_counts.status` | `draft`, `in_progress`, `completed` | **enforced** (CHECK in `0039`) | `server/routes/inventory.js` (complete / auto-complete claim) | Cycle-count lifecycle. The atomic `status='completed' WHERE status='in_progress'` claim guards double-apply. (Distinct from `count_type`, listed below.) No shared constant yet. |
| `cycle_count_workers.role` / `.status` | role: `counter`, `auditor`, `reconciler`; status: `pending`, `submitted` | **enforced** (CHECK in `0061`) | `server/routes/inventory.js` (cycle-count assignments) | Blind-count workflow roles + per-assignment submission state. No shared constant yet. |
| `roles.parent_role` | `worker`, `admin` (nullable) | **enforced** (CHECK in `0088`) | `server/permissions.js`, `server/routes/admin.js` roles | The base role a custom permission-role inherits from. No shared constant yet. |
| `system_flags.key` / `.value` | key: `email_mode`; value: `real`, `redirect`, `suppress` | **enforced** (CHECKs in `0221`) | `server/email.js` (`dbEmailMode`) | DB-wide ops flag, never written by the app. The stage sync workflow sets `email_mode=suppress` after scrubbing a prod copy so a stage server (even with `NODE_ENV=production`) never emails anyone. Empty on prod. The flag can only make email *safer*: `suppress`/`redirect` override the env mode; `real` never un-suppresses a non-prod env. |
...
- `onboarding_rates_confirmed_at` / `onboarding_timezone_confirmed_at` (ISO
  timestamp string, default `''`) — set when an admin explicitly confirms the
  default pay rate / company time zone from the dashboard onboarding checklist
  (`OnboardingChecklist.jsx`). The rates step also counts as done once the company
  default-rate history has an admin-made row.
...
- `hours_rules` (JSON policy document, default `''`) — the configurable
  ...
  **`rules[]` — the open-ended half (M4).** Everything above is fixed-slot: one
  ...
  | `rules[].applies` (sick_value) | `sick` \| `vacation` \| `both` (default) |

  **`sick_value` (Time Off Value)** — `hours` ≥ 0 (0 = "a leave day on these days
  is worth nothing"; blank/negative drops the rule). Full-day leave is valued:
  scheduled shift → matching Time Off Value rule → Regular Shift default **only on
  a company working day** (weekdays with `standardHours`; Mon–Fri when none), so
  weekends/days off pay 0. Each day's total leave is capped at that day's value.

  **Schedule rounding** (`reference: 'schedule'`): a punch within `graceMin` of the
  scheduled edge counts as the scheduled edge; beyond the grace the ACTUAL punch is
  rounded to `intervalMin` in the chosen direction (toward_worker never pays to the
  scheduled end for an early leave). An overnight schedule (end < start) always
  ends next day. `auto_break` is evaluated once per worker-day over the day's total
  hours and allocated to the day's longest entry.
