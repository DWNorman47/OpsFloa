...
| `qbo_bill_range_pay.status` | `billed`, `baseline` | **enforced** (CHECK `chk_qbo_bill_range_pay_status` in `0214`, default `billed`) | `server/constants/qboEnums.js` (`QBO_BILL_LEDGER_STATUSES`), `server/routes/qbo.js` (`billLabor`) | `billed` = on a QuickBooks bill. `baseline` = lazily seeded cutover row: pay on a day/week billed BEFORE the ledger existed (entries stamped before `0211` was applied, per `schema_migrations.applied_at`) is recorded as already billed at the amount computed when first met, instead of being billed again. Both count as billed in the diff. Companion column `credit_cents` (≤ 0, CHECK in `0214`): a negative adjustment held back because the bill would net negative — nets against the next bill, or `POST /api/qbo/bill-credits/record` applies it after a manual vendor credit. |
| `qbo_bill_pushes.status` | `pending`, `posted` | **enforced** (CHECK `chk_qbo_bill_pushes_status` in `0214`) | `server/constants/qboEnums.js` (`QBO_BILL_PUSH_STATUSES`), `server/routes/qbo.js` (push-bills, `reconcilePendingBills`) | Bill outbox. Written `pending` with the Intuit requestid + payload BEFORE `createBill`; `posted` when entry stamps + ledger commit in one transaction. A `pending` row is replayed with the same requestid on the next push (Intuit returns the existing bill) — never billed twice; the worker isn't billed again until it resolves. A definite QuickBooks rejection deletes it. |
