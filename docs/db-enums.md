...
| `invoices.source` | `scratch`, `estimate`, `project` | **enforced** (CHECK in `0149`) | `server/constants/projectMoneyEnums.js`, `server/routes/invoices.js` | Which creation path produced it — blank, copied from an accepted estimate, or generated from project time + expenses. |
