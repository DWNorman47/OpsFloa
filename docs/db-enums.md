...
| `daily_checklist_assignments.schedule_type` | `every`, `ordinal`, `date` | **enforced** (CHECK in `0175`) | `server/constants/dailyChecklistEnums.js` (`DAILY_CHECKLIST_ASSIGNMENT_SCHEDULES`), `server/routes/dailyChecklist.js` | When a Project Daily checklist seeds a day. `every` = every worked day; `ordinal` = only the Nth worked day (`ordinal_target`); `date` = only a specific `scheduled_date`. NOT NULL DEFAULT `every`. An ordinal + a date assignment that resolve to the same worked day both match at assembly → their items combine (deduped) into that day. Companion `carryover` bool (per assignment) sets whether its incomplete items roll to the next day; `daily_checklist_items.carryover` (DEFAULT true) records it per assembled item. |
