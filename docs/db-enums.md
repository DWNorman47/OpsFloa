...
| `purchase_orders.status` | `draft`, `submitted`, `partial`, `received`, `cancelled` | **enforced** (CHECK in `0050`) | `server/routes/inventory.js` (PO receive/cancel flows) | Inventory purchase-order lifecycle. Drives received-basis materials cost + committed-cost rollup (cancelled excluded). No shared constant yet. |
