...
| `daily_checklist_recurring_items.mode` + `daily_checklist_items.mode` | `shared`, `individual` | **enforced** (CHECK in `0172`) | `server/constants/dailyChecklistEnums.js` (`DAILY_CHECKLIST_ITEM_MODES`), `server/routes/dailyChecklist.js` | Project Daily authoring. `shared` = one row, everyone with the matching team-member-type sees + checks the same state; `individual` = each matching person gets a private check state in `daily_checklist_item_user_state`. NOT NULL DEFAULT `shared` (existing rows keep today's shared meaning). Pairs with the nullable scope columns `recurring.project_id` (NULL = default/all projects) and `recurring.role_id` / `items.role_id` (NULL = all types) — FKs, not enums. |
